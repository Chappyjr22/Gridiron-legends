...
const IDENTITY_FIELDS=[{key:'city',label:'City / school',max:28},{key:'name',label:'Team name',max:24},{key:'abbr',label:'Abbreviation',max:4}];
...
function ensureUI(){
 ...
 .team-editor-select{width:100%;min-height:44px;background:#172d46;border:2px solid #567493;color:#f5edcf;padding:8px;font:800 11px "Courier New",monospace}.team-editor-preview{display:grid;place-items:center;gap:8px;min-height:190px;text-align:center;background:linear-gradient(#173451,#0a1824);border:2px solid #45657c;padding:10px}.team-editor-badge{width:64px;height:64px;display:grid;place-items:center;background:var(--p);border:5px solid var(--a);color:#fff;font:900 18px Impact,"Arial Black",sans-serif;box-shadow:4px 4px 0 #000}.team-editor-sprite{width:128px;height:128px;image-rendering:pixelated;background:transparent}.team-editor-uniform{position:relative;width:94px;height:112px}.team-editor-pants{position:absolute;left:33px;top:82px;width:30px;height:28px;background:var(--pants);border-left:4px solid var(--stripe);border-right:4px solid var(--stripe);clip-path:polygon(0 0,100% 0,100% 100%,60% 100%,50% 45%,40% 100%,0 100%)}.team-editor-helmet{position:absolute;left:29px;top:1px;width:38px;height:30px;border-radius:20px 20px 10px 10px;background:var(--helmet);border:3px solid #091117;box-shadow:inset 4px 0 rgba(255,255,255,.12)}.team-editor-helmet:after{content:"";position:absolute;left:15px;top:0;width:5px;height:28px;background:var(--stripe)}.team-editor-jersey{position:absolute;left:17px;top:33px;width:62px;height:54px;clip-path:polygon(13% 0,87% 0,100% 24%,84% 37%,78% 100%,22% 100%,16% 37%,0 24%);background:var(--jersey)}.team-editor-jersey:after{content:"";position:absolute;left:28px;top:0;width:6px;height:54px;background:var(--stripe)}
 ...
 .team-editor-body{flex:1;overflow:hidden}.team-editor-panel{overflow:auto;overscroll-behavior:contain}.team-editor-brand summary{min-height:44px;padding:12px 0;font:18px var(--sports-face);cursor:pointer}.team-editor-select{margin-bottom:8px}
 ...
 .team-editor-identity{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}
 .team-editor-identity label{display:grid;gap:4px;min-width:0;color:#ffda54;font:16px var(--sports-face)}
 .team-editor-identity input{box-sizing:border-box;min-width:0;width:100%;min-height:44px;padding:8px;border:1px solid #4b687a;background:#071017;color:#f5edcf;font:16px Arial,sans-serif}
 .team-editor-identity label:last-child{max-width:150px}
 @media(orientation:landscape) and (max-height:500px){
  ...
  .team-editor-panel{padding:8px}.team-editor-colors{grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.team-editor-color{grid-template-columns:1fr;padding:5px;position:relative}.team-editor-color input[type=color]{width:100%;height:44px}.team-editor-color input[type=text]{display:none}.team-editor-color label{font-size:16px;margin:0}.team-editor-note{font-size:12px}
  .team-editor-color{gap:5px}
  .team-editor-panel:first-child{display:grid;grid-template-columns:minmax(0,1fr) 90px;grid-template-rows:44px auto;align-content:start;gap:7px}
  .team-editor-panel:first-child>.label{display:none}.team-editor-select{grid-column:1/-1;margin:0}
  .team-editor-pref{display:flex;flex-direction:column;align-items:stretch;justify-content:center;gap:5px;margin:0;padding:5px;align-self:start}
  .team-editor-preview{min-height:0;padding:0;border:0;background:transparent;gap:0;align-content:start}
  .team-editor-preview .team-editor-badge,.team-editor-preview>strong{display:none}
  .team-editor-sprite{width:90px;height:90px}.team-editor-head #team-editor-status{font-size:12px;line-height:1.2;max-width:45%;text-align:center}
  .team-editor-preview .team-editor-uniform{transform:scale(.6);margin:-22px 0}
 }
 @media(orientation:landscape) and (max-height:340px){
  .team-editor-sprite{width:64px;height:64px}
  .team-editor-tabs{margin-bottom:5px}.team-editor-section-title{display:none}
  .team-editor-color{padding:4px;gap:3px}.team-editor-color label{font-size:15px;line-height:16px}
 }
 ...
 dialog.innerHTML=`<div class="team-editor-card"><header class="team-editor-head"><div><h2>Team Editor</h2><p>Brand + home / away / alternate uniforms</p></div><span id="team-editor-status" role="status" class="experience-note"></span><button type="button" id="team-editor-close" aria-label="Close team editor">✕</button></header><div class="team-editor-body"><section class="team-editor-panel"><label class="label" for="team-editor-select">Team</label><select id="team-editor-select" class="team-editor-select"></select><div class="team-editor-pref"><label for="team-uniform-preference">Game uniform</label><select id="team-uniform-preference"><option value="auto">Auto</option><option value="home">Home</option><option value="away">Away</option><option value="alternate">Alternate</option></select></div><div class="team-editor-preview" id="team-editor-preview"><div id="team-editor-badge" class="team-editor-badge"></div><canvas id="team-editor-sprite" class="team-editor-sprite" width="128" height="128" aria-label="Player uniform preview" hidden></canvas><div id="team-editor-uniform" class="team-editor-uniform"><div class="team-editor-pants"></div><div class="team-editor-helmet"></div><div class="team-editor-jersey"></div></div><strong id="team-editor-name"></strong></div></section><section class="team-editor-panel"><div class="team-editor-tabs" id="team-editor-tabs">${UNIFORM_VARIANTS.map(v=>`<button type="button" data-uniform-variant="${v}">${v}</button>`).join('')}</div><div class="team-editor-section-title">Uniform colors</div><div class="team-editor-colors" id="team-editor-uniform-colors"></div><details class="team-editor-brand"><summary>Team name &amp; branding</summary><div class="team-editor-identity" id="team-editor-identity"></div><div class="team-editor-colors" id="team-editor-brand-colors"></div></details><p class="team-editor-note">Auto follows the home/away schedule in Career. Quick Play uses Home for your team. Choose Alternate to wear your alternate set. Save team keeps all three sets.</p></section></div><footer class="team-editor-actions"><button type="button" id="team-editor-cancel">Cancel</button><button type="button" id="team-editor-save" class="team-editor-save">Save team</button></footer></div>`;
 document.body.appendChild(dialog);
 spriteImage.addEventListener('load',()=>{if(dialog.open)updatePreview();});
 ...
 for(const b of el('team-editor-tabs').querySelectorAll('button'))b.onclick=()=>{if(!captureDraft()){el('team-editor-status').textContent='Check the team name, abbreviation, and colors before changing sets.';return;}activeVariant=b.dataset.uniformVariant;renderVariant();};
}
function currentTeam(){return drafts.get(el('team-editor-select')?.value)||null;}
function captureDraft(){const team=currentTeam(),values=valuesFromControls();if(!team||!values||identityError(values.identity))return false;Object.assign(team,values.identity);team.colors={...team.colors,...values.brand};team.uniforms[activeVariant]={...team.uniforms[activeVariant],...values.uniform};team.uniformPreference=values.preference;return true;}
...
function renderBrand(){const team=currentTeam();if(!team)return;
 const identity=el('team-editor-identity');identity.innerHTML=IDENTITY_FIELDS.map(f=>`<label>${f.label}<input type="text" data-team-identity="${f.key}" value="${escapeHtml(team[f.key])}" maxlength="${f.max}" spellcheck="false" ${f.key==='abbr'?'autocapitalize="characters"':''}></label>`).join('');
 for(const input of identity.querySelectorAll('input'))input.oninput=updatePreview;
const root=el('team-editor-brand-colors');root.innerHTML=BRAND_FIELDS.map(f=>control(f,team.colors[f.key],`brand`)).join('');wireControls(root);}
function readIdentity(){return Object.fromEntries(IDENTITY_FIELDS.map(f=>{let value=el('team-editor-identity').querySelector(`[data-team-identity="${f.key}"]`).value.trim().replace(/\s+/g,' ');if(f.key==='abbr')value=value.toUpperCase();return [f.key,value];}));}
function identityError(identity){
 if(!identity.city||!identity.name)return 'Enter a city / school and team name.';
 if(!/^[\p{L}\p{N} .’'-]+$/u.test(identity.city+identity.name))return 'Use letters, numbers, spaces, apostrophes, periods, or hyphens in names.';
 if(identity.city.length>28||identity.name.length>24)return 'Keep city / school to 28 characters and team name to 24.';
 if(!/^[A-Z0-9]{2,4}$/.test(identity.abbr))return 'Use 2–4 letters or numbers for the abbreviation.';
 return '';
}
function valuesFromControls(){
 const brand=readFields(BRAND_FIELDS,'brand',el('team-editor-brand-colors'));const uniform=readFields(UNIFORM_FIELDS,`uniform.${activeVariant}`,el('team-editor-uniform-colors'));if(!brand||!uniform)return null;return {brand,uniform,identity:readIdentity(),preference:el('team-uniform-preference').value};
}
function updatePreview(){const team=currentTeam(),values=valuesFromControls();if(!team||!values)return;const preview=el('team-editor-preview');preview.style.setProperty('--p',values.brand.primary);preview.style.setProperty('--a',values.brand.accent);preview.style.setProperty('--jersey',values.uniform.jersey);preview.style.setProperty('--helmet',values.uniform.helmet);preview.style.setProperty('--stripe',values.uniform.stripe);preview.style.setProperty('--pants',values.uniform.pants);const canvas=el('team-editor-sprite'),ready=spriteImage.complete&&spriteImage.naturalWidth>0;
 canvas.hidden=!ready;el('team-editor-uniform').hidden=ready;
 if(ready){const sheet=makeTeamSpriteSheet(values.uniform,2),ctx=canvas.getContext('2d');ctx.clearRect(0,0,128,128);ctx.imageSmoothingEnabled=false;ctx.drawImage(sheet,128,0,64,64,0,0,128,128);}
 el('team-editor-badge').textContent=values.identity.abbr;el('team-editor-name').textContent=`${values.identity.city} ${values.identity.name} · ${activeVariant.toUpperCase()}`;}
...
async function saveTeam(){
 ...
 const error=identityError(values.identity);if(error){el('team-editor-status').textContent=error;return;}
 ensureUniformVariants(team);Object.assign(team,values.identity);team.colors={...team.colors,...values.brand};team.uniforms[activeVariant]={...team.uniforms[activeVariant],...values.uniform};team.uniformPreference=values.preference;
 const target=state.franchise.teams.find(t=>t.id===team.id);const previous={...target};
 Object.assign(target,JSON.parse(JSON.stringify({...values.identity,colors:team.colors,uniforms:team.uniforms,uniformPreference:team.uniformPreference,uniformSchemaVersion:2,uniformsCustomized:true})));
 try{const ok=await state.persist?.(state.franchise,target);if(ok===false)throw new Error('Save failed');state.onApplied?.(target);el('team-editor-select').selectedOptions[0].textContent=`${target.city} ${target.name}`;el('team-editor-status').textContent='Team saved · all three sets';}catch{for(const key of Object.keys(target))if(!(key in previous))delete target[key];Object.assign(target,previous);el('team-editor-status').textContent='Could not save. Your edits are still here. Try again.';}
}
