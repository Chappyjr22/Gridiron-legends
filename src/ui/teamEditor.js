...
import {openUniformMaskDev} from './uniformMaskDev.js';
...
function ensureUI(){
 ...
 .editor-subhead{font:900 11px "Courier New",monospace;text-transform:uppercase;margin:11px 0 7px}.uniform-pref{display:grid;grid-template-columns:auto 1fr;gap:8px;align-items:center;margin-top:11px}.uniform-pref select{min-height:40px;background:#fff;color:#101010;border:0;padding:6px;font:900 11px "Courier New",monospace}.editor-note{font-size:8px;line-height:1.4;opacity:.82;margin:10px 0 0}.editor-status{text-align:center;font-size:9px;min-height:14px;margin-top:8px}.qa-callout{border:2px dashed rgba(255,255,255,.65);padding:7px 8px;margin-top:8px;font-size:8px;line-height:1.45;background:rgba(6,30,86,.22)}.qa-callout button{width:100%;min-height:40px;margin-top:7px;background:#ffcf39;color:#111;border:3px solid #fff}
...
function showDetail(){
 ...
 el('team-editor-content').innerHTML=`<div class="team-detail"><section><h3 id="team-detail-name" class="team-detail-name">${escapeHtml(teamLabel(team))}</h3><div class="team-detail-meta"><span class="team-chip">${escapeHtml(team.abbr)}</span><span class="team-chip">${escapeHtml(titleCase(team.conference||'League'))}</span>${team.division?`<span class="team-chip">${escapeHtml(titleCase(team.division))}</span>`:''}</div><fieldset class="editor-panel"><legend>Uniforms</legend><div class="sprite-variant-row">${UNIFORM_VARIANTS.map(v=>`<button type="button" class="sprite-card" data-variant="${v}"><canvas id="uniform-preview-${v}" width="64" height="64"></canvas><b>${v}</b></button>`).join('')}</div><div class="editor-subhead">Pose QA</div><div class="sprite-state-row">${PREVIEW_STATES.map(s=>`<div class="sprite-state"><canvas id="sprite-state-${s.key}" width="64" height="64"></canvas><span>${s.label}</span></div>`).join('')}</div><div class="qa-callout"><b>MASK TEST:</b> Give Helmet, Jersey, Pants and Stripe four obviously different colors. If a pose is wrong, use the temporary pixel editor to paint the exact equipment pixels.<button type="button" id="open-mask-dev">Open Pixel Mask Dev Editor</button></div></fieldset></section><section><fieldset class="editor-panel"><legend>Customize</legend><div class="uniform-tabs">${UNIFORM_VARIANTS.map(v=>`<button type="button" class="uniform-tab" data-variant="${v}">${v}</button>`).join('')}</div><div class="editor-subhead">${activeVariant.toUpperCase()} uniform</div><div id="team-uniform-colors" class="color-grid"></div><div class="editor-subhead">Team branding</div><div id="team-brand-colors" class="color-grid"></div><div class="uniform-pref"><label for="team-uniform-preference">Game uniform</label><select id="team-uniform-preference"><option value="auto">Auto</option><option value="home">Home</option><option value="away">Away</option><option value="alternate">Alternate</option></select></div><div id="team-editor-status" class="editor-status"></div></fieldset></section></div>`;
 const brand=el('team-brand-colors');brand.innerHTML=BRAND_FIELDS.map(f=>control(f,team.colors[f.key],'brand')).join('');wireControls(brand);el('team-uniform-preference').value=team.uniformPreference||'auto';el('team-uniform-preference').onchange=updatePreview;
 for(const button of document.querySelectorAll('[data-variant]'))button.onclick=()=>{activeVariant=button.dataset.variant;renderVariantControls();};
 el('open-mask-dev').onclick=()=>openUniformMaskDev(()=>variantValues(team,activeVariant),()=>renderSpritePreviews());
 ...
 el('team-editor-restore').onclick=()=>{delete team.uniforms;delete team.uniformPreference;ensureUniformVariants(team);showDetail();};el('team-editor-save').onclick=saveTeam;renderVariantControls();
}
async function saveTeam(){
 ...
 const ok=await state.persist?.(state.franchise,team);if(ok===false){el('team-editor-status').textContent='Could not save team colors.';return;}state.onApplied?.(team);el('team-editor-status').textContent=`${titleCase(activeVariant)} uniform saved.`;renderSpritePreviews();
}
export function openTeamEditor(franchise,selectedTeamId,persist,onApplied){ensureUI();state={franchise,persist,onApplied,selectedTeamId,activeConference:null};for(const team of franchise?.teams||[])ensureUniformVariants(team);showBrowser();el('team-editor-dialog').showModal();}
