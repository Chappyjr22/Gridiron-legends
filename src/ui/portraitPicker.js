...
export function openPortraitPicker(player,t,onApply){
 team=t;apply=onApply;choice={...player,skin:Number(player.skin??2),portrait:Number.isInteger(player.portrait)?player.portrait:0};
 el('portrait-tone').value=choice.skin;render();el('portrait-dialog').showModal();
}
function render(){
 el('portrait-grid').innerHTML=Array.from({length:7},(_,i)=>`<button type="button" aria-label="Face ${i+1}" aria-pressed="${choice.portrait===i}" data-face="${i}"><canvas width="64" height="64" aria-hidden="true"></canvas><span>${i+1}</span></button>`).join('');
