...
export function renderMyTeam(c){
 ...
 el('my-team-roster').innerHTML=playingRoster(team).map(p=>`<button class="roster-card" data-player-id="${esc(p.id)}"><span>#${p.number} · ${esc(p.position)}${p.id===c.playerId?' · YOU':''}</span><strong>${esc(name(p))}</strong><span>${p.rating} OVR · AGE ${p.age}</span></button>`).join('');
 for(const b of el('my-team-roster').querySelectorAll('button'))b.onclick=()=>openPlayer(c,playingRoster(team).find(p=>p.id===b.dataset.playerId));
}
