...
export function renderCareerStats(c){
 ...
 el('league-stat-coverage').textContent=`${scope.value==='playoffs'?'Playoffs only.':'Regular season only.'} ${data.covered}/${data.games} completed games have box scores. Your team uses recorded plays. Opponents and other teams use simulated production. Earlier untracked games show —, not zero. GP counts tracked appearances.`;
}
