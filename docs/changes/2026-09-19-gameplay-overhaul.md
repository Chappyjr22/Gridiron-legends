...
- Blocking assigns the closest available player first, holds at actual contact, and releases after a rating-dependent interval. Designed runs get time to reach the line; linemen no longer receive a 20-second win. Defense mixes man and bounded zone assignments; difficulty adjusts reactions, pursuit, blitzing and tackle timing while preserving player ratings.
...
- Production build, 40 gameplay checks and 12 overhaul checks pass locally. The earlier career/college/depth, save, data, audio, cloud and uniform suites also passed.
...
- Observed results included a 61-yard catch-and-run ended with a dive, two one-yard completions and two negative-yardage inside runs. A 45-rep deterministic harness probe across Easy/Medium/Hard confirmed that the same immediate receiver tap is not universally a touchdown. A follow-up run probe exposed distant blockers claiming nearby defenders. After correcting assignment and contact holds, equal-rating Medium reps produced short gains and occasional outside breakaways instead of universal losses. These are small balance samples, not proof of season-long balance.
