...
The first CI run passed both mobile aim cases but exposed a timing error in an existing replay test: its asynchronous wait predicate returned a truthy Promise, allowing Next Rep to be tapped before the 400ms result guard expired. The test now polls the awaited boolean. The game’s duplicate-tap protection is unchanged.
