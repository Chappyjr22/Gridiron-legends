...
`db/migrations/0001_cloud_saves.sql` is versioned, applied transactionally to preview. Not yet applied to production.
...
Browser tests stub service responses for mobile layout, email outage and two-device conflict handling. These do not prove real email delivery or iPhone login.
...
2. Apply the migration to production; add the exact production domain to trusted origins; disable development localhost origins through Neon settings.
