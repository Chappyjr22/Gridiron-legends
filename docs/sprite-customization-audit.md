...
Gameplay loads and SHA-256 checks the exact PNG bytes used for image decoding against the corresponding mask. Dimension/range validation prevents coordinate mismatches. A missing or mismatched mask falls back to the previous uniform renderer instead of blocking play. Recoloring happens only while rebuilding cached team/skin sheets, not per animation frame. Team Editor shows the actual recolored sprite and adds pants for home, away and alternate sets; existing saves default to white pants. Career menu sprites use the selected uniform too. Original source art, existing skin-tone recoloring, simulation, and career progress remain unchanged. Uniform mask preservation tests apply before the existing skin-tone pass.
...
The complete masks are marked `reviewed:true` for the four uniform materials only, with an explicit `reviewScope`. The original three-pose pilot remains `reviewed:false` as a historical standalone artifact.
