...
for(const [width,height,kind,schoolId] of [[844,304,'teammate','college-bluegrass'],[667,375,'challenge',null],[844,390,'teammate',null]])test(`weekly choices and stakes at ${width}x${height}`,async({browser})=>{
 ...
 await page.getByLabel('Weekly preparation',{exact:true}).selectOption(kind);
 ...
 await page.reload();await resume(page);await expect(page.getByLabel('Weekly preparation',{exact:true})).toHaveValue(kind);
 ...
 await page.reload();await resume(page);await expect(page.getByLabel('Weekly preparation',{exact:true})).toBeDisabled();
 ...
 await page.getByRole('button',{name:'Continue to career',exact:true}).click();await expect(page.getByLabel('Weekly preparation',{exact:true})).toHaveValue('personal');
