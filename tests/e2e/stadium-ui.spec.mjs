...
for(const viewport of [{width:844,height:304},{width:844,height:390},{width:932,height:430},{width:1366,height:768}]){
 test(`stadium and career fit ${viewport.width}x${viewport.height}`,async({browser})=>{
  ...
  await page.getByLabel('Career starting point',{exact:true}).selectOption('pro');await page.getByLabel('Player name',{exact:true}).fill('Jacob Chapman');await page.getByRole('button',{name:'Next',exact:true}).click();
  ...
  await assertInViewport(page,'#career-menu-open,#career-play,#career-open-player,.career-nav button');
