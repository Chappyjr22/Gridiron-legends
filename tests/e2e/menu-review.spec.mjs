...
for(const viewport of [{width:844,height:304},{width:932,height:430}]){
 test(`complete secondary menu review ${viewport.width}`,async({browser})=>{
  ...
  await page.locator('#setup-screen [data-open-guide]').tap();await shot('guide');await page.locator('#practice-guide-close').tap();await page.locator('#btn-setup-close').tap();
  await page.locator('#btn-new-game').tap();await shot('quickplay');await page.locator('.roster-details summary').tap();await shot('quick-roster');await page.locator('#btn-setup-close').tap();
  ...
  await page.locator('#btn-extras').tap();await page.locator('#btn-start-editor').tap();await shot('formation-lab');await page.locator('#btn-done-edit').tap();
  await page.locator('[data-open-account]').first().tap();await shot('account');await page.locator('#cloud-close').tap();
  await page.locator('#btn-career').tap();await shot('career-gateway');await page.locator('#career-my-careers').tap();await shot('career-list');await page.locator('#career-list-back').tap();
