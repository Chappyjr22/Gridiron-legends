...
for(const viewport of [{width:844,height:304},{width:932,height:430}]){
 test(`complete secondary menu review ${viewport.width}`,async({browser})=>{
  ...
  await page.locator('#setup-screen [data-open-guide]').tap();await shot('guide');await expect(page.locator('#practice-guide-heading')).toBeInViewport({ratio:1});await expect(page.locator('#practice-guide-close')).toBeInViewport({ratio:1});await page.locator('#practice-guide-close').tap();await page.locator('#btn-setup-close').tap();
