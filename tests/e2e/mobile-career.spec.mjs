...
for(const viewport of [{width:844,height:304},{width:844,height:390},{width:932,height:370},{width:390,height:740}]){
 test(`college player panel scrolls with fixed navigation at ${viewport.width}x${viewport.height}`,async({browser})=>{
  ...
  await expect(page.locator('#upgrade-status')).toHaveText('0 points · Level up to earn 3');
  ...
  await page.getByRole('button',{name:'Touchdowns',exact:true}).tap();
  ...
  await page.locator('[data-league-view=standings]').tap();await expect(page.locator('#career-standings')).toBeInViewport();
