...
test('desktop field fits the available width and guided practice remains playable',async({page})=>{
 await page.setViewportSize({width:1363,height:936});await page.goto('/');await page.locator('#btn-menu-settings').click();await page.locator('#setup-screen [data-settings-page=help]').click();await page.locator('#setup-screen [data-open-guide]').click();await expect(page.locator('#practice-guide')).toBeVisible();await page.locator('#practice-guide-start').click();
...
test('short landscape settings retain sound controls and keyboard focus',async({browser})=>{
 ...
 await page.locator('#pause-overlay [data-settings-page=sound]').tap();await page.locator('#pause-overlay [data-audio-mute]').tap();await expect(page.locator('#pause-overlay [data-audio-mute]')).toHaveAttribute('aria-pressed','false');
 await page.locator('#pause-overlay [data-settings-page=gameplay]').tap();await page.locator('#pause-overlay [data-mode="direct"]').tap();await expect(page.locator('#pause-overlay [data-mode="direct"]')).toHaveAttribute('aria-pressed','true');
