...
import {proProjection} from '../src/career/development.js';
...
for(const win of [true,false]){
 ...
 c=C.parseCareer(JSON.stringify(c));assert.ok(c);
 const projected=proProjection(C.careerPlayer(c),c);
 ...
 assert.deepEqual(C.careerPlayer(c).attributes,projected.attributes);assert.equal(C.careerPlayer(c).rating,projected.overall);assert.deepEqual(c.collegeArchive.attributes,before.attrs);assert.equal(c.points,Math.floor(before.points/3)+Math.floor((before.xp+Math.floor(before.points%3*100/3))/100));
